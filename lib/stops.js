...
function generateRecordXform(agencyId, agencyName, layerName) {
  return (rec, _, next) => {
    ...
    // add any optional (addendum) data to the stop record
    var addendum = {};

    if (rec.direction && rec.direction.length > 0) {
      addendum.direction = rec.direction;
      addendum.dir = rec.direction.charAt(0).toUpperCase();  // assumes Northblah, Southblah, etc...
    }

    if (rec.position && rec.position.length > 0) {
      addendum.position = rec.position;
    }

    doc.setAddendum("gtfs", addendum);

    // advance to the next phase of the pipeline
    next(null, doc);
  };
}
